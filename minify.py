...
JS_INPUT = os.path.join(SRC_DIR, "neiki-social-bar.js")
CSS_INPUT = os.path.join(SRC_DIR, "neiki-social-bar.css")

JS_OUTPUT = os.path.join(OUTPUT_DIR, "neiki-social-bar.js")
JS_MIN_OUTPUT = os.path.join(OUTPUT_DIR, "neiki-social-bar.min.js")
JS_MIN_TEMP = os.path.join(OUTPUT_DIR, "neiki-social-bar.min.temp.js")
CSS_OUTPUT = os.path.join(OUTPUT_DIR, "neiki-social-bar.css")
CSS_MIN_OUTPUT = os.path.join(OUTPUT_DIR, "neiki-social-bar.min.css")

BANNER = "/* neiki-social-bar 1.0.0 | MIT */\n"
...
def embed_css(js_source, css_text):
    if CSS_MARKER not in js_source:
        raise RuntimeError("Could not find EMBEDDED_CSS marker in src/neiki-social-bar.js")
...
def main():
    ...
    if not minified_js.startswith("/* neiki-social-bar"):
